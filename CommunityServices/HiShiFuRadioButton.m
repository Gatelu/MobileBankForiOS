...
@interface HiShiFuRadioButton()
-(void)defaultInit;
-(void)otherButtonSelected:(id)sender;
-(void)handleButtonTap:(id)sender;
@end

@implementation HiShiFuRadioButton
...
-(void)defaultInit{
    ...
    _button.frame = CGRectMake(0, 0,30, 40);
...
//    [_button setTitleColor:[UIColor redColor] forState:UIControlStateHighlighted];
    ...
    [HiShiFuRadioButton registerInstance:self];
}
#pragma mark - Manage Instances
//添加选中的RadioButton
+(void)registerInstance:(HiShiFuRadioButton*)radioButton{
    if(!rb_instances){
...
-(void)handleButtonTap:(id)sender{
    ...
    [HiShiFuRadioButton buttonSelected:self];
}

#pragma mark - Class level handler
//RadioButton选中后调用的事件
+(void)buttonSelected:(HiShiFuRadioButton*)radioButton{
    // 通知观察者
    if (rb_observers) {
        //获得观察者对象
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            HiShiFuRadioButton *button = [rb_instances objectAtIndex:i];
