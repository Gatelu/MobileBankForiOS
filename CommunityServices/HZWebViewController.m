...
- (void)viewDidLoad
{
  ...
  if(self.mode == HZWebBrowserModeModal){
    ...
    if(HZFSystemVersion >= 7.0){
      ...
      webViewRect = CGRectMake(webViewRect.origin.x, 20, webViewRect.size.width, webViewRect.size.height);
    }
    else{
      webViewRect = CGRectMake(webViewRect.origin.x, 0, webViewRect.size.width, webViewRect.size.height);
    }
